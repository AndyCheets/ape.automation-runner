...
public sealed class WorkflowDefinitionParser
{
    public WorkflowDefinition Parse(string yaml)
    {
        ...
        int version = int.TryParse(
            root.GetValueOrDefault("version")?.ToString(),
            out int parsed
        )
            ? parsed
            : 0;
        ...
        if (root.TryGetValue("steps", out object? stepsObject) && stepsObject is List<object> stepList)
        {
            foreach (object stepObj in stepList)
            {
                ...
                int? timeoutSeconds = int.TryParse(
                    step.GetValueOrDefault("timeoutSeconds")?.ToString(),
                    out int timeout
                )
                    ? timeout
                    : null;
                ...
                steps.Add(
                    new WorkflowStepDefinition(
                        step.GetValueOrDefault("stepKey")?.ToString() ?? string.Empty,
                        step.GetValueOrDefault("taskType")?.ToString() ?? string.Empty,
                        timeoutSeconds,
                        config
                    )
                );
